...
export function installDevices({architecture,solids,box,cylinder,planeTexture,canvasTexture,materials,uiTicket,uiFood}){
 ...
 const advert=canvasTexture(800,480,(c,w,h)=>{c.fillStyle='#ef4e23';c.fillRect(0,0,w,h);c.fillStyle='#fff';c.font='bold 105px Segoe UI';c.fillText('PISELL',78,230);c.font='26px Segoe UI';c.fillText('Powering the Way You Do Business',82,290);});
 ...
 function screen(parent,w,h,x,y,z,tilt,ui,body=graphite,back=false){const group=new THREE.Group();group.position.set(x,y,z);group.rotation.x=tilt;parent.add(group);box(w,h,.032,0,0,0,body,group);box(w-.012,h-.012,.007,0,0,.018,black,group);planeTexture(ui,w-.037,h-.038,new THREE.Vector3(0,0,.023),0,group);if(back)planeTexture(logo,w*.40,h*.20,new THREE.Vector3(0,h*.2,-.018),Math.PI,group);return group;}
 ...
 function g2(g,ui){
  ...
  const rear=new THREE.Group();rear.position.set(0,.34,-.12);rear.rotation.y=Math.PI;g.add(rear);screen(rear,.218,.136,0,0,.019,-.25,uiFood,white);
